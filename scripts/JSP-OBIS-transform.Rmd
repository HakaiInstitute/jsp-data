...
fish_lab_data <- read_csv("https://raw.githubusercontent.com/HakaiInstitute/jsp-data/master/data/fish_lab_data.csv", guess_max = 20000)

fish_field_data <- read_csv("https://raw.githubusercontent.com/HakaiInstitute/jsp-data/master/data/fish_field_data.csv", guess_max = 20000) %>% 
  select(ufn, seine_id, species, fork_length_field, weight_field)
...
write_csv(event, here::here("processed_data", "OBIS", "event.csv"))
```
...
occurrence <- bind_rows(all_fish_caught, fish_retained_data) %>% 
  mutate(basisOfRecord = "Human Observation",
...
```{r}
#TODO: get worrms to return all the data I need for taxonomy
(species <- unique(occurrence$scientificName))
class(species)
wm_records_name(name = (unique(occurrence$scientificName)))

# I went directly to the WoRMS website (http://www.marinespecies.org/) to download the full taxonomic levels for the salmon species I have and put the WoRMS output (species_matched.xls) table in this project directory which is read in below and joined with the occurrence table

species_matched <- readxl::read_excel(here::here("processed_data", "OBIS", "species_matched.xls"))

occurrence <- left_join(occurrence, species_matched, by = c("scientificName" = "ScientificName")) %>% 
  select(occurrenceID, scientificNameID = LSID, basisOfRecord, scientificName, eventID, occurrenceStatus = occurenceStatus, Kingdom, Phylum, Class, Order, Family, Genus, Species)

write_csv(occurrence, here::here("processed_data", "OBIS", "occurrence.csv"))
...
#TODO add fork length from NVS vocab
fish_length <- fish_data %>%
  mutate(occurrenceID = paste0("hakai-jsp-", ufn)) %>% 
  select(occurrenceID, eventID = seine_id, fork_length, weight) %>% 
  mutate(measurementType = "Fork_length", measurementValue = fork_length) %>% 
  select(eventID, occurrenceID, measurementType, measurementValue) %>% 
  mutate(measurementTypeID = "http://vocab.nerc.ac.uk/collection/P01/current/FL01XX01/") %>% 
...
fish_weight <- fish_data %>% 
  ...
  mutate(measurementTypeID = "http://vocab.nerc.ac.uk/collection/P01/current/SPWGXX01/") %>% 
...
write_csv(measurementOrFact, here::here("processed_data", "OBIS", "extendedMeasurementOrFact.csv"))

# fish_mof <- fish_data %>%
#   mutate(occurrenceID = paste0("hakai-jsp-", ufn)) %>%
#   select(occurrenceID, eventID = seine_id, fork_length, weight) %>%
#     rename(length = fork_length) %>%
#   pivot_longer(length:weight,
