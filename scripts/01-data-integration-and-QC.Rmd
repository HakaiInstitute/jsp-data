...
1
zoop_tax_samples <- read_sheet(mi_slug, sheet = "zoop_tax")  %>% 
...
stomach_samples <- read_sheet(mi_slug, sheet = "stomach_samples")
carcasses_samples <- read_sheet(mi_slug, sheet = "carcasses_samples")
isotope_samples <- read_sheet(mi_slug, sheet = "isotope_samples")
otolith_samples <- read_sheet(mi_slug, sheet = "otolith_samples")
extra_muscle_samples <- read_sheet(mi_slug, sheet ="extra_muscle_samples")
fatty_acid_samples <- read_sheet(mi_slug, sheet = "fatty_acid_samples")
scale_samples <- read_sheet(mi_slug, sheet = "scale_samples")
rna_dna_samples <- read_sheet(mi_slug, sheet = "rna_dna_samples")
rna_pathogen_samples <- read_sheet(mi_slug, sheet ="rna_pathogen_samples")
sealice_microbiome_samples <- read_sheet(mi_slug, sheet ="sea_lice_microbiome_samples") %>% distinct(sample_id, .keep_all = TRUE)
fish_packages <- read_sheet(mi_slug, sheet = "fish_packages")
