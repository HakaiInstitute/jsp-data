...
qu39_ctd <- tabledap("HakaiWaterPropertiesInstrumentProfileProvisional", 
                          url = url,
                          'station="QU39"'
                          ) %>% 
  mutate(device_firmware = as.numeric(device_firmware))

jsp_ctd_casts <- tabledap("HakaiWaterPropertiesInstrumentProfileProvisional", 
                          url = url,
                          'cruise=~"(DISOCKEYE|DI_W,DISOCKEYE|DI_C,DISOCKEYE|JSSOCKEYE)"'
                          )

ctd_all <- bind_rows(qu39_ctd, jsp_ctd_casts)
rm(qu39_ctd, jsp_ctd_casts)
...
salmon_dm_no_keys <- dm(survey_data, seine_data, fish_field_data, fish_lab_data, 
                        ...
                        ysi, ctd_all, dna_samples, stock_id,
...
#TODO sort out  DE374N1, DE111N1 because they appear to be on land
...
stocks <- left_join(stock_id, dna_samples)

jsp_catch_and_bio_data_complete <- survey_seines_fish %>% 
  left_join(stocks, by = "ufn") %>% 
  left_join(stock_id) %>% 
  left_join(combined_motile_lice) %>% 
