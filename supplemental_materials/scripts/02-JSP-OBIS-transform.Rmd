...
all_fish_not_retained <-  full_join(seines_total_long, seines_taken_long, by = c("seine_id", "scientificName")) %>% 
  drop_na() %>% 
...
lice_occ <- fish_data |> 
  select(ufn, eventID = seine_id, "Lepeophtheirus salmonis" = motile_lep, "Caligus clemensi" = motile_caligus) |>
  pivot_longer(cols = `Lepeophtheirus salmonis`:`Caligus clemensi`, names_to = "scientificName", values_to = "organismQuantity")|> 
  drop_na() |> 
...
occurrence <- bind_rows(all_fish_not_retained, fish_retained_data) %>% 
  ...
  left_join(select(event, eventDate, eventID)) |> 
